...
public class AddBinary {
    public String addBinary(String a, String b) {
        ...
        for (int i = 0; i < Math.min(a.length(), b.length()); i++) {
            int x = sumOfChars(aBuilder.charAt(i), bBuilder.charAt(i), mind);
            switch (x) {
                case 0: {
                    c.append('0');
                    break;
        ...
        for (int i = Math.min(a.length(), b.length()); i < Math.max(a.length(), b.length()); i++) {
            int x = (a.length() > b.length()) ? sumOfChars(aBuilder.charAt(i), '0', mind) : sumOfChars('0', bBuilder.charAt(i), mind);
            switch (x) {
                case 0: {
                    c.append('0');
                    break;
